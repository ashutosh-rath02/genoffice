...
import type { AgentToolDef } from '@threadnote/agent-core'
import {
...
describe('Codex app-server bridge', () => {
  it('automatically selects the newest complete desktop-managed Codex install', async () => {
    const root = await mkdtemp(join(tmpdir(), 'threadnoteoffice-codex-discovery-'))
    try {
  ...
  it('migrates a stale saved hash path to the current managed install', async () => {
    const root = await mkdtemp(join(tmpdir(), 'threadnoteoffice-codex-migration-'))
    try {
  ...
  it('confines Codex tool reads to the temp dir through a permission profile', () => {
    ...
    const params = codexThreadStartParams(config, '/tmp/threadnoteoffice-codex-x', 'profile')
    expect(params).not.toHaveProperty('sandbox')
    expect(params.approvalPolicy).toBe('never')
    expect(params.config).toEqual({
      default_permissions: 'threadnoteoffice',
      permissions: {
        threadnoteoffice: {
          filesystem: { ':minimal': 'read', '/tmp/threadnoteoffice-codex-x': 'read' },
        },
      },
    })
    expect(
  ...
  it('parses text and JSON-encoded ThreadnoteOffice tool arguments', () => {
    expect(
      parseCodexAppServerTurn(
        JSON.stringify({
