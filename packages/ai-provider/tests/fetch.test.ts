...
describe('aiFetch', () => {
  it('identifies the client to gateways that flag anonymous traffic', async () => {
    ...
    expect(headers.get('user-agent')).toBe('ThreadnoteOffice')
  ...
  it('lets the host refine the user agent and never overrides an explicit one', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response('ok'))
    vi.stubGlobal('fetch', fetchMock)
    setAiUserAgent('ThreadnoteOffice/1.2.3')
    await aiFetch('https://x/', {})
    expect(sentHeaders(fetchMock).get('user-agent')).toBe('ThreadnoteOffice/1.2.3')
  ...
  it('passes the same identified request to the rescue fetch', async () => {
    ...
    await aiFetch('https://x/', {})
    expect(sentHeaders(rescue).get('user-agent')).toBe('ThreadnoteOffice')
  })
