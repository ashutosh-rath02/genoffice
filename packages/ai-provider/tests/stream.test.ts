...
import type { AgentToolCall } from '@threadnote/agent-core'
...
describe('streamForProvider: temperature policy', () => {
  ...
  // issue : every model in the OpenAI BYOK dropdown is GPT-5.x,
...
describe('streamForProvider: anthropic', () => {
  ...
  it('never sends an empty assistant content array when history has edits-only replies', async () => {
    ...
    // on Anthropic (threadnoteoffice#12 / #22 class of multi-turn failures).
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        okResponse(
    ...
    for (const model of ['claude-opus-4-7', 'gpt-5.2']) {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({ 'X-Agent-Type': 'threadnoteoffice' }),
        }),
      )
    }
  })
