...
/** Engine patch plus the picture-bullet source the op layer lands as a media part first */
export type EditParagraphFormatPatch = ParagraphFormatPatch & {
...
): Array<{ index: number; patch: EditParagraphFormatPatch }> {
  const out: Array<{ index: number; patch: EditParagraphFormatPatch }> = []
  edited.forEach((p, i) => {
    const patch: EditParagraphFormatPatch = {
      ...(p.bullet
        ? {
            bullet: p.bullet,
            ...(p.bullet === 'char' && p.bulletChar ? { bulletChar: p.bulletChar } : {}),
            ...(p.bullet === 'char' && p.bulletChar && p.bulletFont
              ? { bulletFont: p.bulletFont }
              : {}),
            ...(p.bullet === 'blip' && p.bulletImage ? { bulletImage: p.bulletImage } : {}),
          }
        : {}),
      ...(p.numType != null && p.bullet !== 'char' && p.bullet !== 'blip'
        ? { numType: p.numType }
        : {}),
