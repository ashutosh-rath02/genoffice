...
function emuRect(op: Op, field = 'box'): EmuRect {
  ...
  for (const k of ['x', 'y', 'cx', 'cy'] as const)
    requireFinite(b[k], op.op as string, `${field}.${k}`)
  if (b.cx < 0 || b.cy < 0) {
    throw new GuidedError(`op "${op.op}": "${field}.cx/cy" must be >= 0 (EMU).`)
  }
  // EMU are integers (ST_Coordinate); models happily compute thirds and halves
  return { x: Math.round(b.x), y: Math.round(b.y), cx: Math.round(b.cx), cy: Math.round(b.cy) }
}
