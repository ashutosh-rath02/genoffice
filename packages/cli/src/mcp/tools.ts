...
const GUI_OPEN =
  'A file open in a ThreadnoteOffice tab is refused without force (the editor would overwrite the edit on its next save).'
...
export const TOOLS: ToolSpec[] = [
  ...
  },
  {
    ...
    command: 'convert',
    description:
      'Convert a document to another format with the ThreadnoteOffice engines (pdf, docx, xlsx, pptx, md, html, csv; the `to` description lists the routes). Targets that need layout (to pdf, docx to html, html to docx) start a hidden ThreadnoteOffice process for 1-6 s. Output defaults to the input name with the new extension.',
  ...
  },
  {
    ...
    command: 'create',
    description:
      'Print a document (md, html, docx, xlsx, pptx) to a new PDF with the ThreadnoteOffice renderer, in a hidden ThreadnoteOffice process.',
    fixed: ['--type', 'pdf'],
    options: [
  ...
  },
  {
    ...
    verb: 'apply',
    description:
      'Edit a .docx with a batch of ops (the op reference: guide docs, or the threadnoteoffice://guide/docs resource). ' +
      READ_THEN_APPLY +
  ...
  },
  {
    ...
    images: true,
    description:
      'One PNG per slide (960x540 at scale 1 for 16:9), written to a directory and returned as images so you can look at the pages. Starts a hidden ThreadnoteOffice process (a few seconds). Use after building or editing a deck; slide renders one page.',
    positionals: [{ key: 'file', description: 'path of the .pptx' }],
    options: [
  ...
  },
  {
    ...
    images: true,
    description:
      'One PNG per page of a document (docx, xlsx, pptx, pdf, md, html) as the ThreadnoteOffice renderer lays it out, written to a directory and returned as images. Starts a hidden ThreadnoteOffice process. page renders one page; grid adds a contact sheet of every page.',
    positionals: [{ key: 'file', description: 'path of the document' }],
    options: [
      { key: 'out', description: 'directory for the PNGs (required)', required: true },
      'page',
  ...
  },
  {
    ...
    plainText: true,
    description:
      'The op reference an agent reads before writing ops: guide("docs"), guide("sheets") or guide("slides") list the groups; topic narrows to one group or one op. guide("slides", "design") is the staged deck workflow and guide("slides", "spec") the outline and page spec format (deck_start returns both). Also available as threadnoteoffice://guide/* resources.',
    positionals: [
  ...
  },
  {
    ...
    readOnly: true,
    description:
      'Which cloud features are configured in ThreadnoteOffice: web search, image search, image generation, media understanding. Check once before planning photos or live facts; everything else runs locally.',
  },
  {
    name: 'search',
    command: 'search',
    ...
    readOnly: true,
    description:
      'Web search, or image search with images=true, through the provider configured in ThreadnoteOffice (the query leaves the machine). Image results carry imageUrl, width and height for addPicture / insert_image.',
  ...
  },
  {
    ...
    openWorld: true,
    description:
      'Generate an image from a prompt with the provider configured in ThreadnoteOffice and save it; the result names the real format. ref images steer edits (background removal, upscale).',
  ...
  },
  {
    ...
    command: 'open',
    description:
      'Open a document in the ThreadnoteOffice app for the user (starts the app if needed), optionally selecting a slide, element, block, range or page. Only when the user asks to see the file: an open tab makes later *_apply calls refuse to write.',
    positionals: [{ key: 'file', description: 'path of the document' }],
