...
export const openCommand: CommandDef = {
  name: 'open',
  summary:
    'Open a document in the ThreadnoteOffice app (starts the app if needed); with a target, also select that spot for the user.',
  usage:
  ...
  async run(args, ctx) {
    ...
    if (!endpoint) {
      ...
      if (!endpoint) {
        throw new CliError(
          EXIT.app,
          'ThreadnoteOffice started but did not publish its control endpoint',
          undefined,
          {
    ...
    return {
      summary: target
        ? `${path}: showing ${describe(target)} in ThreadnoteOffice`
        : `opened ${path} in ThreadnoteOffice`,
...
async function spawnApp(path: string, ctx: CommandContext): Promise<string> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.once('error', (err) =>
      reject(new CliError(EXIT.app, `failed to start ThreadnoteOffice: ${err.message}`)),
    )
    child.once('spawn', () => {
      child.unref()
      resolve()
...
export function parseTarget(args: ParsedArgs, path: string): ControlTarget | undefined {
  ...
  throw new CliError(
    ...
    { reason: 'unsupported', suggestion: 'run `threadnoteoffice open <file>` without a target' },
  )
}
