 ...
 * HTML and Markdown print, Word ↔ HTML) run in the ThreadnoteOffice binary itself through its
 * `--headless-export` entry: Dock hidden, no window, one export, exit. threadnoteoffice
...
export async function exportViaApp(
  ...
  if (!launch) {
    throw new CliError(EXIT.app, 'ThreadnoteOffice app not found (needed for this conversion)', {
      hint: 'install ThreadnoteOffice, or set THREADNOTE_OFFICE_APP_BIN to its executable',
    })
  }
  const args = [
    ...launch.args,
  ...
  opts.log?.(`starting ThreadnoteOffice for ${target} export`)
  ...
  if (exported && (code === 0 || timedOut)) {
    if (timedOut) opts.log?.('export finished but ThreadnoteOffice had to be terminated on quit')
    return { outputPath, summary: envelope.summary ?? `exported to ${outputPath}` }
  }
  if (timedOut) {
    throw new CliError(
      EXIT.conversion,
      `ThreadnoteOffice did not finish the export within ${Math.round((opts.timeoutMs ?? DEFAULT_TIMEOUT_MS) / 1000)}s`,
      {
        app: launch.command,
      },
    )
  ...
  const message =
    ...
    (tail ||
      (code === 0
        ? 'ThreadnoteOffice exited without writing the file; this ThreadnoteOffice version may not support --headless-export'
        : `ThreadnoteOffice exited with code ${code}`))
