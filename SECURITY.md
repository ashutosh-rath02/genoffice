...
[private vulnerability reporting](https://github.com/ashutosh-rath02/threadnote/security/advisories/new)
for the Threadnote project. Do not open public issues for security reports.
...
- Every `shell.openExternal` call goes through a single shared gate
  (`@threadnote/electron-utils` → `safeExternalUrl`) that parses the URL and
...
- No API keys are hardcoded. AI requests use the provider selected in Settings;
  any user-supplied keys stay in the OS-level settings store.
