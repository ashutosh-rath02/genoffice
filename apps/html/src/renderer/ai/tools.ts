import type { AgentSkill, AgentToolCall, AgentToolDef, ToolExecution } from '@threadnote/agent-core'
          ...
          return planBrief(spec, currentSignal).then(async (drafted) => {
            ...
            return {
              output: `Brief confirmed (${mode}).${edited}\n${briefSummary(decision.brief)}\n${
                pinned
                  ? 'The brief is now pinned in <head> (meta name="threadnoteoffice:brief"). '
