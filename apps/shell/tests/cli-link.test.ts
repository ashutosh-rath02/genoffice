...
describe('threadnoteoffice launcher file', () => {
  it('lives in the threadnoteoffice auth directory, overridable like auth.json', () => {
    ...
    expect(launcherFilePath({ THREADNOTE_OFFICE_AUTH_DIR: '/tmp/x' })).toBe(
      join('/tmp/x', 'launcher'),
    )
  })
  ...
  it('writes one line, creates the directory, and only rewrites on change', () => {
    ...
    expect(
      writeLauncherFile(file, '/Applications/ThreadnoteOffice.app/Contents/Resources/cli'),
    ).toBe(true)
    expect(readFileSync(file, 'utf-8')).toBe(
      '/Applications/ThreadnoteOffice.app/Contents/Resources/cli\n',
    )
    ...
    expect(
      writeLauncherFile(file, 'C:\\Programs\\ThreadnoteOffice\\resources\\threadnoteoffice'),
    ).toBe(true)
    expect(readFileSync(file, 'utf-8')).toBe(
      'C:\\Programs\\ThreadnoteOffice\\resources\\threadnoteoffice\n',
    )
  })

  it('treats dmg and AppImage mounts as temporary', () => {
    expect(
      isEphemeralInstall('/Volumes/ThreadnoteOffice/ThreadnoteOffice.app/Contents/Resources', {}),
    ).toBe(true)
    expect(isEphemeralInstall('/tmp/.mount_GenOfxyz/resources', {})).toBe(true)
    expect(
      isEphemeralInstall('/opt/ThreadnoteOffice/resources', { APPIMAGE: '/home/u/G.AppImage' }),
    ).toBe(true)
    expect(isEphemeralInstall('/Applications/ThreadnoteOffice.app/Contents/Resources', {})).toBe(
      false,
    )
  })
})
