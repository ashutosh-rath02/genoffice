...
export const strings = {
  zh: {
    ...
    intgHeroTitle: '在 AI 助手里使用 Threadnote Office',
    intgHeroDesc:
    ...
    intgStep1Update: 'Threadnote Office 升级带来新版 skill 时，对应助手旁会出现「更新」按钮。',
    ...
    intgOtherFolderDesc: '选择它的 skills 目录，Threadnote Office 把文件写进去。',
    ...
    intgExample3: '把 budget.xlsx 转成 PDF，并在 Threadnote Office 里打开',
    intgStep2Note: '助手会自己调用 threadnoteoffice 命令行，你不需要输入任何命令。',
    intgCliPartTitle: 'CLI · 命令行 + skill',
    intgCliPartDesc:
      '适合能运行终端命令的助手（Claude Code、Codex、Cursor 等）。安装 skill 后，助手会自己调用 threadnoteoffice 命令行创建、转换、读取和编辑文件，你不需要输入任何命令。',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP 是 AI 助手调用外部工具的通用协议。助手不能运行命令（如 Claude Desktop），或你想让它直接操作 Threadnote Office 窗口时用这条路。下面两种接法任选一种。',
    intgMcpStdioTitle: '由助手启动（推荐）',
    intgMcpStdioDesc:
      '助手自己拉起 threadnoteoffice mcp，不需要在这里开任何开关，Threadnote Office 也不用开着。功能与 CLI 完全相同。',
    intgMcpHttpTitle: '本地 HTTP 服务',
    intgMcpHttpDesc:
      '在 Threadnote Office 内运行，助手通过网址连接，可以看着它在 Word 编辑器里逐步生成。需要 Threadnote Office 保持打开；目前只支持 Word。',
    intgMcpHttpAdvanced: '高级：其他地址与日志',
    intgMcpTryNote: '助手会通过 MCP 调用 Threadnote Office，你不需要输入任何命令。',
    ...
    intgCliTitle: '高级：命令行 threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · 已可在终端直接使用（{path}）',
    intgCliNotOnPath:
      'threadnoteoffice {v} · 尚未加入终端 PATH。助手仍能通过 ~/.threadnoteoffice/launcher 找到它，skill 不受影响。若想自己在终端输入 threadnoteoffice，运行一次：',
    ...
    intgCopied: '已复制',
    intgCliEphemeral: '请先把 Threadnote Office 拖到「应用程序」，再使用命令行。',
    intgCopyPath: '复制路径',
    intgCliNeedsUpdate: '此 skill 需要命令行 {v} 或更新版本，请更新 Threadnote Office。',
    intgLoading: '正在检测…',
    ...
    onbStarHint: '如果你喜欢 Threadnote Office，欢迎到 GitHub 给我们一个 Star。',
    setEmail: '邮箱',
    setNotLoggedIn: '未登录',
    setViewUsage: '查看用量',
    setChange: '更改',
    ...
    onbTitle1: '欢迎使用 Threadnote Office',
  ...
  en: {
    ...
    settings: 'Settings',
    setSecAccount: 'Optional AI account',
    ...
    intgHeroTitle: 'Use Threadnote Office from your AI assistant',
    intgHeroDesc:
    ...
    intgStep1Update:
      'When a Threadnote Office update ships a newer skill, an Update button appears next to that assistant.',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherDesc:
      'Pick either of these two ways. They all install the same skill, so there is no need to do more than one.',
    intgOtherFolderTitle: 'Any assistant that reads SKILL.md folders',
    intgOtherFolderDesc: 'Pick its skills folder and Threadnote Office writes the file there.',
    ...
    intgExample3: 'Convert budget.xlsx to PDF and open it in Threadnote Office',
    intgStep2Note:
      'The assistant runs the threadnoteoffice command line itself; you never have to type it.',
    intgCliPartTitle: 'CLI · command line + skill',
    intgCliPartDesc:
      'For assistants that can run terminal commands (Claude Code, Codex, Cursor and others). Once the skill is installed, the assistant calls the threadnoteoffice command line itself to create, convert, read and edit files; you never type a command.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP is the common protocol AI assistants use to call external tools. Use it when your assistant cannot run commands (Claude Desktop, for example) or when you want it to drive the Threadnote Office window directly. Pick either of the two ways below.',
    intgMcpStdioTitle: 'Started by the assistant (recommended)',
    intgMcpStdioDesc:
      'The assistant launches threadnoteoffice mcp itself: nothing to switch on here, and Threadnote Office does not need to be open. Same features as the CLI.',
    intgMcpHttpTitle: 'Local HTTP server',
    intgMcpHttpDesc:
      'Runs inside Threadnote Office; the assistant connects to a URL and you can watch it build the document step by step in the Word editor. Threadnote Office must stay open; Word documents only for now.',
    intgMcpHttpAdvanced: 'Advanced: other endpoints and log',
    intgMcpTryNote: 'The assistant calls Threadnote Office through MCP; you never type a command.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'Advanced: threadnoteoffice command line',
    intgCliReady: 'threadnoteoffice {v} · ready in your terminal ({path})',
    intgCliNotOnPath:
      "threadnoteoffice {v} · not on your terminal's PATH. Assistants still find it through ~/.threadnoteoffice/launcher, so the skill works. To type threadnoteoffice yourself, run this once:",
    ...
    intgCopied: 'Copied',
    intgCliEphemeral: 'Drag Threadnote Office into Applications first, then use the command line.',
    intgCopyPath: 'Copy path',
    intgCliNeedsUpdate: 'This skill needs command line {v} or newer; update Threadnote Office.',
    ...
    starPromptTitleN: "You've opened {n} documents with Threadnote Office",
    starPromptBody:
      'Threadnote Office is free and open source. A star on GitHub is the best way to support the team.',
    starPromptGo: 'Star on GitHub',
    starPromptDone: 'Already starred',
    starPromptLater: 'Maybe later',
    onbStarHint: 'If you like Threadnote Office, give us a star on GitHub.',
    setEmail: 'Email',
    ...
    newTab: 'New tab',
    // First-run onboarding
    onbTitle1: 'Welcome to Threadnote Office',
    onbSubtitle1: 'Your documents, on your terms',
    onbBody1:
      'Create and edit locally. Share selected files with your Threadnote workspace when you choose.',
  ...
  ja: {
    ...
    intgHeroTitle: 'AI アシスタントから Threadnote Office を使う',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Threadnote Office の更新で新しいスキルが届くと、そのアシスタントの横に「更新」ボタンが表示されます。',
    ...
    intgExample3: 'budget.xlsx を PDF に変換して Threadnote Office で開いて',
    intgStep2Note:
      'threadnoteoffice コマンドラインはアシスタントが自分で実行します。あなたが入力する必要はありません。',
    intgCliPartTitle: 'CLI · コマンドライン + skill',
    intgCliPartDesc:
      'ターミナルコマンドを実行できるアシスタント（Claude Code、Codex、Cursor など）向け。skill をインストールすると、アシスタントが自分で threadnoteoffice コマンドラインを呼び出してファイルを作成・変換・読み取り・編集します。コマンドを入力する必要はありません。',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP は AI アシスタントが外部ツールを呼び出すための共通プロトコルです。アシスタントがコマンドを実行できない場合（Claude Desktop など）や、Threadnote Office のウィンドウを直接操作させたい場合に使います。下の 2 つの方法からどちらか 1 つを選んでください。',
    intgMcpStdioTitle: 'アシスタントが起動する（推奨）',
    intgMcpStdioDesc:
      'アシスタントが threadnoteoffice mcp を自分で起動します。ここで何かをオンにする必要はなく、Threadnote Office を開いておく必要もありません。機能は CLI と同じです。',
    intgMcpHttpTitle: 'ローカル HTTP サーバー',
    intgMcpHttpDesc:
      'Threadnote Office 内で動作し、アシスタントは URL で接続します。Word エディター上で文書が段階的に生成される様子を見られます。Threadnote Office を開いたままにする必要があり、現時点では Word のみ対応です。',
    intgMcpHttpAdvanced: '詳細：その他のエンドポイントとログ',
    intgMcpTryNote:
      'アシスタントが MCP 経由で Threadnote Office を呼び出します。コマンドを入力する必要はありません。',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: '上級：コマンドライン threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · ターミナルで使えます（{path}）',
    intgCliNotOnPath:
      'threadnoteoffice {v} · ターミナルの PATH にありません。アシスタントは ~/.threadnoteoffice/launcher 経由で見つけられるため、スキルは動作します。自分で threadnoteoffice を入力したい場合は一度だけ次を実行：',
    ...
    intgCopied: 'コピーしました',
    intgCliEphemeral:
      '先に Threadnote Office を「アプリケーション」に移してから、コマンドラインを使ってください。',
    intgCopyPath: 'パスをコピー',
    intgCliNeedsUpdate:
      'このスキルにはコマンドライン {v} 以降が必要です。Threadnote Office を更新してください。',
    intgLoading: '確認中…',
    ...
    starPromptTitleN: 'Threadnote Office で {n} 件のドキュメントを開きました',
    starPromptBody:
      'Threadnote Office は無料のオープンソースソフトウェアです。GitHub のスターがチームへの何よりの応援になります。',
    starPromptGo: 'スターを付ける',
    starPromptDone: 'スター済み',
    starPromptLater: 'あとで',
    onbStarHint: 'Threadnote Office が気に入ったら、GitHub でスターをお願いします。',
    setEmail: 'メール',
    ...
    newTab: '新しいタブ',
    // First-run onboarding
    onbTitle1: 'Threadnote Office へようこそ',
  ...
  ko: {
    ...
    intgHeroTitle: 'AI 어시스턴트에서 Threadnote Office 사용하기',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Threadnote Office 업데이트에 새 스킬이 포함되면 해당 어시스턴트 옆에 업데이트 버튼이 나타납니다.',
    ...
    intgOtherFolderDesc: '해당 skills 폴더를 선택하면 Threadnote Office가 파일을 기록합니다.',
    ...
    intgExample3: 'budget.xlsx를 PDF로 변환해서 Threadnote Office에서 열어 줘',
    intgStep2Note:
      'threadnoteoffice 명령줄은 어시스턴트가 직접 실행하므로 여러분이 입력할 필요가 없습니다.',
    intgCliPartTitle: 'CLI · 명령줄 + skill',
    intgCliPartDesc:
      '터미널 명령을 실행할 수 있는 어시스턴트(Claude Code, Codex, Cursor 등)용입니다. skill을 설치하면 어시스턴트가 스스로 threadnoteoffice 명령줄을 호출해 파일을 만들고 변환하고 읽고 편집합니다. 명령을 직접 입력할 필요가 없습니다.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP는 AI 어시스턴트가 외부 도구를 호출하는 공통 프로토콜입니다. 어시스턴트가 명령을 실행할 수 없거나(예: Claude Desktop) Threadnote Office 창을 직접 조작하게 하려면 이 방식을 사용하세요. 아래 두 방법 중 하나를 선택하면 됩니다.',
    intgMcpStdioTitle: '어시스턴트가 실행(권장)',
    intgMcpStdioDesc:
      '어시스턴트가 threadnoteoffice mcp를 직접 실행합니다. 여기서 켤 것이 없고 Threadnote Office가 열려 있을 필요도 없습니다. 기능은 CLI와 같습니다.',
    intgMcpHttpTitle: '로컬 HTTP 서버',
    intgMcpHttpDesc:
      'Threadnote Office 안에서 실행되며 어시스턴트가 URL로 연결합니다. Word 편집기에서 문서가 단계별로 만들어지는 과정을 볼 수 있습니다. Threadnote Office를 열어 두어야 하며 현재는 Word만 지원합니다.',
    intgMcpHttpAdvanced: '고급: 기타 주소와 로그',
    intgMcpTryNote:
      '어시스턴트가 MCP를 통해 Threadnote Office를 호출합니다. 명령을 직접 입력할 필요가 없습니다.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: '고급: 명령줄 threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · 터미널에서 바로 사용 가능({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · 터미널 PATH에 없습니다. 어시스턴트는 ~/.threadnoteoffice/launcher로 찾을 수 있어 스킬은 정상 작동합니다. 직접 threadnoteoffice를 입력하려면 한 번만 실행:',
    ...
    starPromptTitleN: 'Threadnote Office로 문서를 {n}개 열어보셨네요',
    starPromptBody:
      'Threadnote Office는 무료 오픈 소스 소프트웨어입니다. GitHub 스타 하나가 팀에게 가장 큰 응원이 됩니다.',
    starPromptGo: '스타 누르기',
    starPromptDone: '이미 눌렀어요',
    starPromptLater: '나중에',
    onbStarHint: 'Threadnote Office가 마음에 들면 GitHub에서 스타를 눌러 주세요.',
    setEmail: '이메일',
    ...
    newTab: '새 탭',
    // First-run onboarding
    onbTitle1: 'Threadnote Office에 오신 것을 환영합니다',
  ...
  fr: {
    ...
    intgHeroTitle: 'Utiliser Threadnote Office depuis votre assistant IA',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Quand une mise à jour de Threadnote Office apporte un skill plus récent, un bouton Mettre à jour apparaît à côté de cet assistant.',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'Choisissez son dossier skills et Threadnote Office y écrit le fichier.',
    ...
    intgExample3: 'Convertis budget.xlsx en PDF et ouvre-le dans Threadnote Office',
    intgStep2Note:
      "L'assistant lance lui-même la ligne de commande threadnoteoffice ; vous n'avez jamais à la saisir.",
    intgCliPartTitle: 'CLI · ligne de commande + skill',
    intgCliPartDesc:
      'Pour les assistants capables de lancer des commandes dans le terminal (Claude Code, Codex, Cursor, etc.). Une fois le skill installé, l’assistant appelle lui-même la ligne de commande threadnoteoffice pour créer, convertir, lire et modifier les fichiers ; vous ne tapez jamais de commande.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP est le protocole commun que les assistants IA utilisent pour appeler des outils externes. Utilisez-le si votre assistant ne peut pas lancer de commandes (Claude Desktop, par exemple) ou si vous voulez qu’il pilote directement la fenêtre Threadnote Office. Choisissez l’une des deux méthodes ci-dessous.',
    intgMcpStdioTitle: 'Lancé par l’assistant (recommandé)',
    intgMcpStdioDesc:
      'L’assistant lance lui-même threadnoteoffice mcp : rien à activer ici, et Threadnote Office n’a pas besoin d’être ouvert. Mêmes fonctions que la CLI.',
    ...
    intgCliTitle: 'Avancé : ligne de commande threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · prêt dans votre terminal ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · absent du PATH de votre terminal. Les assistants le trouvent quand même via ~/.threadnoteoffice/launcher, le skill fonctionne donc. Pour taper threadnoteoffice vous-même, exécutez ceci une fois :',
    ...
    intgCopied: 'Copié',
    intgCliEphemeral:
      "Glissez d'abord Threadnote Office dans Applications, puis utilisez la ligne de commande.",
    intgCopyPath: 'Copier le chemin',
    intgCliNeedsUpdate:
      'Ce skill requiert la ligne de commande {v} ou plus récente ; mettez Threadnote Office à jour.',
    ...
    starPromptTitleN: 'Vous avez ouvert {n} documents avec Threadnote Office',
    starPromptBody:
      'Threadnote Office est gratuit et open source. Une étoile sur GitHub est le meilleur moyen de soutenir l’équipe.',
    starPromptGo: 'Mettre une étoile',
    starPromptDone: 'Déjà fait',
    starPromptLater: 'Plus tard',
    onbStarHint: 'Si Threadnote Office vous plaît, mettez-nous une étoile sur GitHub.',
    setEmail: 'E-mail',
    ...
    // First-run onboarding
    onbTitle1: 'Bienvenue dans Threadnote Office',
  ...
  de: {
    ...
    intgHeroTitle: 'Threadnote Office aus Ihrem KI-Assistenten nutzen',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Bringt ein Threadnote Office-Update einen neueren Skill mit, erscheint neben diesem Assistenten ein Aktualisieren-Button.',
    intgSkillVersion: 'Skill {v}',
    ...
    intgOtherFolderDesc:
      'Wählen Sie seinen Skills-Ordner, und Threadnote Office schreibt die Datei dorthin.',
    ...
    intgExample3: 'Wandle budget.xlsx in PDF um und öffne es in Threadnote Office',
    intgStep2Note:
      'Der Assistent ruft die threadnoteoffice-Befehlszeile selbst auf; Sie müssen sie nie eintippen.',
    intgCliPartTitle: 'CLI · Kommandozeile + Skill',
    intgCliPartDesc:
      'Für Assistenten, die Terminalbefehle ausführen können (Claude Code, Codex, Cursor u. a.). Nach der Installation des Skills ruft der Assistent die threadnoteoffice-Kommandozeile selbst auf, um Dateien zu erstellen, zu konvertieren, zu lesen und zu bearbeiten; Sie tippen nie einen Befehl.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP ist das gemeinsame Protokoll, über das KI-Assistenten externe Werkzeuge aufrufen. Nutzen Sie es, wenn der Assistent keine Befehle ausführen kann (z. B. Claude Desktop) oder wenn er das Threadnote Office-Fenster direkt steuern soll. Wählen Sie einen der beiden Wege unten.',
    intgMcpStdioTitle: 'Vom Assistenten gestartet (empfohlen)',
    intgMcpStdioDesc:
      'Der Assistent startet threadnoteoffice mcp selbst: hier ist nichts einzuschalten, und Threadnote Office muss nicht geöffnet sein. Gleicher Funktionsumfang wie die CLI.',
    ...
    intgCliTitle: 'Erweitert: Befehlszeile threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · im Terminal verfügbar ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · nicht im PATH Ihres Terminals. Assistenten finden es trotzdem über ~/.threadnoteoffice/launcher, der Skill funktioniert also. Um threadnoteoffice selbst einzugeben, führen Sie dies einmal aus:',
    ...
    intgCopied: 'Kopiert',
    intgCliEphemeral:
      'Ziehen Sie Threadnote Office zuerst in „Programme“, dann nutzen Sie die Befehlszeile.',
    intgCopyPath: 'Pfad kopieren',
    intgCliNeedsUpdate:
      'Dieser Skill braucht Befehlszeile {v} oder neuer; aktualisieren Sie Threadnote Office.',
    intgLoading: 'Prüfe …',
    ...
    starPromptTitleN: 'Sie haben {n} Dokumente mit Threadnote Office geöffnet',
    starPromptBody:
      'Threadnote Office ist kostenlos und Open Source. Ein Stern auf GitHub ist die beste Unterstützung für das Team.',
    starPromptGo: 'Stern geben',
    starPromptDone: 'Schon erledigt',
    starPromptLater: 'Später',
    onbStarHint: 'Wenn Ihnen Threadnote Office gefällt, geben Sie uns einen Stern auf GitHub.',
    setEmail: 'E-Mail',
    ...
    newTab: 'Neuer Tab',
    // First-run onboarding
    onbTitle1: 'Willkommen bei Threadnote Office',
  ...
  es: {
    ...
    intgHeroTitle: 'Usa Threadnote Office desde tu asistente de IA',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Cuando una actualización de Threadnote Office traiga un skill más nuevo, aparecerá un botón Actualizar junto a ese asistente.',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'Elige su carpeta de skills y Threadnote Office escribe el archivo ahí.',
    ...
    intgExample3: 'Convierte budget.xlsx a PDF y ábrelo en Threadnote Office',
    intgStep2Note:
      'El asistente ejecuta la línea de comandos threadnoteoffice por sí mismo; nunca tienes que escribirla.',
    intgCliPartTitle: 'CLI · línea de comandos + skill',
    intgCliPartDesc:
      'Para asistentes que pueden ejecutar comandos de terminal (Claude Code, Codex, Cursor y otros). Con el skill instalado, el asistente llama por sí mismo a la línea de comandos threadnoteoffice para crear, convertir, leer y editar archivos; nunca escribes un comando.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP es el protocolo común con el que los asistentes de IA llaman a herramientas externas. Úsalo cuando tu asistente no pueda ejecutar comandos (Claude Desktop, por ejemplo) o cuando quieras que controle directamente la ventana de Threadnote Office. Elige una de las dos formas de abajo.',
    intgMcpStdioTitle: 'Iniciado por el asistente (recomendado)',
    intgMcpStdioDesc:
      'El asistente lanza threadnoteoffice mcp por sí mismo: nada que activar aquí, y Threadnote Office no necesita estar abierto. Mismas funciones que la CLI.',
    ...
    intgCliTitle: 'Avanzado: línea de comandos threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · listo en tu terminal ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · no está en el PATH de tu terminal. Los asistentes lo encuentran igualmente mediante ~/.threadnoteoffice/launcher, así que el skill funciona. Para escribir threadnoteoffice tú mismo, ejecuta esto una vez:',
    ...
    starPromptTitleN: 'Has abierto {n} documentos con Threadnote Office',
    starPromptBody:
      'Threadnote Office es gratuito y de código abierto. Una estrella en GitHub es la mejor forma de apoyar al equipo.',
    starPromptGo: 'Dar una estrella',
    starPromptDone: 'Ya la di',
    starPromptLater: 'Más tarde',
    onbStarHint: 'Si te gusta Threadnote Office, danos una estrella en GitHub.',
    ...
    // First-run onboarding
    onbTitle1: 'Bienvenido a Threadnote Office',
  ...
  th: {
    ...
    intgHeroTitle: 'ใช้ Threadnote Office จากผู้ช่วย AI ของคุณ',
    intgHeroDesc:
    ...
    intgStep1Update:
      'เมื่อการอัปเดต Threadnote Office มาพร้อมสกิลเวอร์ชันใหม่ ปุ่มอัปเดตจะปรากฏข้างผู้ช่วยนั้น',
    ...
    intgOtherFolderDesc: 'เลือกโฟลเดอร์ skills ของผู้ช่วย แล้ว Threadnote Office จะเขียนไฟล์ลงไป',
    ...
    intgExample3: 'แปลง budget.xlsx เป็น PDF แล้วเปิดใน Threadnote Office',
    intgStep2Note: 'ผู้ช่วยจะเรียกใช้บรรทัดคำสั่ง threadnoteoffice เอง คุณไม่ต้องพิมพ์คำสั่งใด ๆ',
    intgCliPartTitle: 'CLI · บรรทัดคำสั่ง + skill',
    intgCliPartDesc:
      'สำหรับผู้ช่วยที่รันคำสั่งเทอร์มินัลได้ (Claude Code, Codex, Cursor และอื่น ๆ) เมื่อติดตั้ง skill แล้ว ผู้ช่วยจะเรียกบรรทัดคำสั่ง threadnoteoffice เองเพื่อสร้าง แปลง อ่าน และแก้ไขไฟล์ คุณไม่ต้องพิมพ์คำสั่งใด ๆ',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP คือโปรโตคอลกลางที่ผู้ช่วย AI ใช้เรียกเครื่องมือภายนอก ใช้เมื่อผู้ช่วยรันคำสั่งไม่ได้ (เช่น Claude Desktop) หรือเมื่อต้องการให้มันควบคุมหน้าต่าง Threadnote Office โดยตรง เลือกวิธีใดวิธีหนึ่งจากสองวิธีด้านล่าง',
    intgMcpStdioTitle: 'ให้ผู้ช่วยเป็นผู้เริ่ม (แนะนำ)',
    intgMcpStdioDesc:
      'ผู้ช่วยจะเรียก threadnoteoffice mcp ขึ้นมาเอง ไม่ต้องเปิดสวิตช์ใดที่นี่ และไม่ต้องเปิด Threadnote Office ไว้ ความสามารถเหมือน CLI ทุกประการ',
    intgMcpHttpTitle: 'เซิร์ฟเวอร์ HTTP ในเครื่อง',
    intgMcpHttpDesc:
      'ทำงานอยู่ใน Threadnote Office ผู้ช่วยเชื่อมต่อผ่าน URL และคุณจะเห็นเอกสารค่อย ๆ ถูกสร้างขึ้นในตัวแก้ไข Word ต้องเปิด Threadnote Office ไว้ตลอด ปัจจุบันรองรับเฉพาะ Word',
    intgMcpHttpAdvanced: 'ขั้นสูง: ที่อยู่อื่นและบันทึก',
    intgMcpTryNote: 'ผู้ช่วยจะเรียก Threadnote Office ผ่าน MCP คุณไม่ต้องพิมพ์คำสั่งใด ๆ',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'ขั้นสูง: บรรทัดคำสั่ง threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · พร้อมใช้ในเทอร์มินัล ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · ยังไม่อยู่ใน PATH ของเทอร์มินัล ผู้ช่วยยังหาเจอผ่าน ~/.threadnoteoffice/launcher สกิลจึงใช้งานได้ตามปกติ หากต้องการพิมพ์ threadnoteoffice เอง ให้รันคำสั่งนี้ครั้งเดียว:',
    ...
    intgCliEphemeral: 'กรุณาลาก Threadnote Office ไปที่ Applications ก่อน แล้วจึงใช้บรรทัดคำสั่ง',
    intgCopyPath: 'คัดลอกพาธ',
    intgCliNeedsUpdate: 'สกิลนี้ต้องใช้บรรทัดคำสั่ง {v} หรือใหม่กว่า โปรดอัปเดต Threadnote Office',
    ...
    starPromptTitleN: 'คุณเปิดเอกสารด้วย Threadnote Office ไปแล้ว {n} ไฟล์',
    starPromptBody:
      'Threadnote Office ฟรีและเป็นโอเพนซอร์ส การกดดาวบน GitHub คือกำลังใจที่ดีที่สุดสำหรับทีม',
    starPromptGo: 'กดดาวเลย',
    starPromptDone: 'กดแล้ว',
    starPromptLater: 'ไว้ทีหลัง',
    onbStarHint: 'ถ้าชอบ Threadnote Office ฝากกดดาวให้เราบน GitHub ด้วยนะ',
    setEmail: 'อีเมล',
    ...
    newTab: 'แท็บใหม่',
    // First-run onboarding
    onbTitle1: 'ยินดีต้อนรับสู่ Threadnote Office',
  ...
  id: {
    ...
    intgHeroTitle: 'Gunakan Threadnote Office dari asisten AI Anda',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Saat pembaruan Threadnote Office membawa skill yang lebih baru, tombol Perbarui muncul di samping asisten itu.',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'Pilih folder skills-nya dan Threadnote Office menulis file di sana.',
    ...
    intgExample3: 'Konversi budget.xlsx ke PDF dan buka di Threadnote Office',
    intgStep2Note:
      'Asisten menjalankan baris perintah threadnoteoffice sendiri; Anda tidak perlu mengetiknya.',
    intgCliPartTitle: 'CLI · baris perintah + skill',
    intgCliPartDesc:
      'Untuk asisten yang bisa menjalankan perintah terminal (Claude Code, Codex, Cursor, dan lainnya). Setelah skill terpasang, asisten memanggil baris perintah threadnoteoffice sendiri untuk membuat, mengonversi, membaca, dan mengedit file; Anda tidak pernah mengetik perintah.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP adalah protokol umum yang dipakai asisten AI untuk memanggil alat eksternal. Gunakan saat asisten Anda tidak bisa menjalankan perintah (misalnya Claude Desktop) atau saat Anda ingin ia mengendalikan jendela Threadnote Office secara langsung. Pilih salah satu dari dua cara di bawah.',
    intgMcpStdioTitle: 'Dijalankan oleh asisten (disarankan)',
    intgMcpStdioDesc:
      'Asisten menjalankan threadnoteoffice mcp sendiri: tidak ada yang perlu dinyalakan di sini, dan Threadnote Office tidak perlu terbuka. Fitur sama dengan CLI.',
    ...
    intgCliTitle: 'Lanjutan: baris perintah threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · siap di terminal Anda ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · belum ada di PATH terminal Anda. Asisten tetap menemukannya lewat ~/.threadnoteoffice/launcher, jadi skill tetap berfungsi. Untuk mengetik threadnoteoffice sendiri, jalankan ini sekali:',
    ...
    intgCopied: 'Disalin',
    intgCliEphemeral: 'Seret Threadnote Office ke Applications dulu, lalu gunakan baris perintah.',
    intgCopyPath: 'Salin path',
    intgCliNeedsUpdate:
      'Skill ini memerlukan baris perintah {v} atau lebih baru; perbarui Threadnote Office.',
    ...
    starPromptTitleN: 'Anda telah membuka {n} dokumen dengan Threadnote Office',
    starPromptBody:
      'Threadnote Office gratis dan open source. Bintang di GitHub adalah dukungan terbaik untuk tim kami.',
    starPromptGo: 'Beri Bintang',
    starPromptDone: 'Sudah',
    starPromptLater: 'Nanti saja',
    onbStarHint: 'Jika Anda menyukai Threadnote Office, beri kami bintang di GitHub.',
    setEmail: 'Email',
    ...
    newTab: 'Tab baru',
    // First-run onboarding
    onbTitle1: 'Selamat datang di Threadnote Office',
  ...
  ru: {
    ...
    intgHeroTitle: 'Используйте Threadnote Office из своего ИИ-ассистента',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Когда обновление Threadnote Office принесёт более новый скилл, рядом с этим ассистентом появится кнопка «Обновить».',
    ...
    intgOtherFolderDesc: 'Выберите его папку skills, и Threadnote Office запишет файл туда.',
    ...
    intgExample3: 'Конвертируй budget.xlsx в PDF и открой в Threadnote Office',
    intgStep2Note:
      'Ассистент сам запускает командную строку threadnoteoffice; вводить её вам не придётся.',
    intgCliPartTitle: 'CLI · командная строка + skill',
    intgCliPartDesc:
      'Для ассистентов, умеющих выполнять команды терминала (Claude Code, Codex, Cursor и другие). После установки skill ассистент сам вызывает командную строку threadnoteoffice, чтобы создавать, конвертировать, читать и редактировать файлы; вам не нужно вводить команды.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP — общий протокол, через который ИИ-ассистенты вызывают внешние инструменты. Используйте его, если ассистент не умеет выполнять команды (например, Claude Desktop) или если хотите, чтобы он напрямую управлял окном Threadnote Office. Выберите один из двух способов ниже.',
    intgMcpStdioTitle: 'Запускает ассистент (рекомендуется)',
    intgMcpStdioDesc:
      'Ассистент сам запускает threadnoteoffice mcp: здесь ничего включать не нужно, и Threadnote Office не обязан быть открыт. Те же возможности, что у CLI.',
    ...
    intgCliTitle: 'Дополнительно: командная строка threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · доступен в терминале ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · отсутствует в PATH терминала. Ассистенты всё равно находят его через ~/.threadnoteoffice/launcher, так что скилл работает. Чтобы вводить threadnoteoffice самостоятельно, выполните один раз:',
    ...
    starPromptTitleN: 'Вы уже открыли {n} документов в Threadnote Office',
    starPromptBody:
      'Threadnote Office — бесплатное ПО с открытым кодом. Звезда на GitHub — лучшая поддержка для команды.',
    starPromptGo: 'Поставить звезду',
    starPromptDone: 'Уже поставлена',
    starPromptLater: 'Позже',
    onbStarHint: 'Если вам нравится Threadnote Office, поставьте нам звезду на GitHub.',
    setEmail: 'Эл. почта',
    ...
    // First-run onboarding
    onbTitle1: 'Добро пожаловать в Threadnote Office',
  ...
  ar: {
    ...
    intgHeroTitle: 'استخدم Threadnote Office من مساعد الذكاء الاصطناعي',
    intgHeroDesc:
    ...
    intgOtherFolderDesc: 'اختر مجلد skills الخاص به وسيكتب Threadnote Office الملف هناك.',
    ...
    intgExample3: 'حوّل budget.xlsx إلى PDF وافتحه في Threadnote Office',
    intgStep2Note: 'المساعد يشغّل سطر أوامر threadnoteoffice بنفسه؛ لن تحتاج إلى كتابته أبداً.',
    intgCliPartTitle: 'CLI · سطر الأوامر + skill',
    intgCliPartDesc:
      'للمساعدين الذين يستطيعون تنفيذ أوامر الطرفية (Claude Code وCodex وCursor وغيرها). بعد تثبيت skill، يستدعي المساعد سطر أوامر threadnoteoffice بنفسه لإنشاء الملفات وتحويلها وقراءتها وتحريرها؛ لن تكتب أي أمر.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP هو البروتوكول المشترك الذي يستخدمه مساعدو الذكاء الاصطناعي لاستدعاء أدوات خارجية. استخدمه إذا كان مساعدك لا يستطيع تنفيذ الأوامر (مثل Claude Desktop) أو إذا أردت أن يتحكم في نافذة Threadnote Office مباشرة. اختر إحدى الطريقتين أدناه.',
    intgMcpStdioTitle: 'يشغّله المساعد (موصى به)',
    intgMcpStdioDesc:
      'يشغّل المساعد threadnoteoffice mcp بنفسه: لا شيء لتفعيله هنا، ولا يلزم أن يكون Threadnote Office مفتوحًا. نفس إمكانات CLI.',
    intgMcpHttpTitle: 'خادم HTTP محلي',
    intgMcpHttpDesc:
      'يعمل داخل Threadnote Office؛ يتصل المساعد عبر عنوان URL ويمكنك مشاهدة المستند يُبنى خطوة بخطوة في محرر Word. يجب أن يبقى Threadnote Office مفتوحًا؛ مستندات Word فقط حاليًا.',
    intgMcpHttpAdvanced: 'متقدم: عناوين أخرى والسجل',
    intgMcpTryNote: 'يستدعي المساعد Threadnote Office عبر MCP؛ لن تكتب أي أمر.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'متقدم: سطر أوامر threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · جاهز في الطرفية ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · غير موجود في PATH الطرفية. ما زال المساعدون يجدونه عبر ~/.threadnoteoffice/launcher، فالمهارة تعمل. لكتابة threadnoteoffice بنفسك، شغّل هذا مرة واحدة:',
    ...
    intgCopied: 'تم النسخ',
    intgCliEphemeral: 'اسحب Threadnote Office إلى مجلد التطبيقات أولاً، ثم استخدم سطر الأوامر.',
    intgCopyPath: 'نسخ المسار',
    intgCliNeedsUpdate: 'تحتاج هذه المهارة إلى سطر الأوامر {v} أو أحدث؛ حدّث Threadnote Office.',
    ...
    starPromptTitleN: 'لقد فتحت {n} من المستندات في Threadnote Office',
    starPromptBody: 'Threadnote Office مجاني ومفتوح المصدر. نجمة على GitHub هي أفضل دعم للفريق.',
    starPromptGo: 'ضع نجمة',
    starPromptDone: 'وضعتها بالفعل',
    starPromptLater: 'لاحقًا',
    onbStarHint: 'إذا أعجبك Threadnote Office، ضع لنا نجمة على GitHub.',
    ...
    // First-run onboarding
    onbTitle1: 'مرحبًا بك في Threadnote Office',
  ...
  pt: {
    ...
    intgHeroTitle: 'Use o Threadnote Office a partir do seu assistente de IA',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Quando uma atualização do Threadnote Office trouxer um skill mais novo, um botão Atualizar aparece ao lado desse assistente.',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'Escolha a pasta de skills dele e o Threadnote Office grava o arquivo lá.',
    ...
    intgExample3: 'Converta budget.xlsx para PDF e abra no Threadnote Office',
    intgStep2Note:
      'O assistente executa a linha de comando threadnoteoffice sozinho; você nunca precisa digitá-la.',
    intgCliPartTitle: 'CLI · linha de comando + skill',
    intgCliPartDesc:
      'Para assistentes que executam comandos de terminal (Claude Code, Codex, Cursor e outros). Com o skill instalado, o assistente chama a linha de comando threadnoteoffice sozinho para criar, converter, ler e editar arquivos; você nunca digita um comando.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP é o protocolo comum que assistentes de IA usam para chamar ferramentas externas. Use-o quando o assistente não puder executar comandos (Claude Desktop, por exemplo) ou quando quiser que ele controle a janela do Threadnote Office diretamente. Escolha uma das duas formas abaixo.',
    intgMcpStdioTitle: 'Iniciado pelo assistente (recomendado)',
    intgMcpStdioDesc:
      'O assistente inicia o threadnoteoffice mcp sozinho: nada para ligar aqui, e o Threadnote Office não precisa estar aberto. Mesmos recursos da CLI.',
    intgMcpHttpTitle: 'Servidor HTTP local',
    intgMcpHttpDesc:
      'Roda dentro do Threadnote Office; o assistente conecta por URL e você acompanha o documento sendo montado passo a passo no editor Word. O Threadnote Office precisa ficar aberto; por enquanto só documentos Word.',
    intgMcpHttpAdvanced: 'Avançado: outros endereços e log',
    intgMcpTryNote: 'O assistente chama o Threadnote Office via MCP; você nunca digita um comando.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'Avançado: linha de comando threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · pronto no seu terminal ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · não está no PATH do seu terminal. Os assistentes ainda o encontram por ~/.threadnoteoffice/launcher, então o skill funciona. Para digitar threadnoteoffice você mesmo, execute isto uma vez:',
    ...
    intgCopied: 'Copiado',
    intgCliEphemeral:
      'Arraste o Threadnote Office para Aplicativos primeiro e depois use a linha de comando.',
    intgCopyPath: 'Copiar caminho',
    intgCliNeedsUpdate:
      'Este skill precisa da linha de comando {v} ou mais recente; atualize o Threadnote Office.',
    ...
    starPromptTitleN: 'Você já abriu {n} documentos com o Threadnote Office',
    starPromptBody:
      'O Threadnote Office é gratuito e de código aberto. Uma estrela no GitHub é a melhor forma de apoiar a equipe.',
    starPromptGo: 'Dar uma estrela',
    starPromptDone: 'Já dei',
    starPromptLater: 'Mais tarde',
    onbStarHint: 'Se você gosta do Threadnote Office, deixe uma estrela no GitHub.',
    setEmail: 'E-mail',
    ...
    newTab: 'Nova guia',
    // First-run onboarding
    onbTitle1: 'Bem-vindo ao Threadnote Office',
  ...
  it: {
    ...
    intgHeroTitle: 'Usa Threadnote Office dal tuo assistente IA',
    intgHeroDesc:
    ...
    intgStep1Update:
      "Quando un aggiornamento di Threadnote Office porta uno skill più recente, accanto a quell'assistente compare un pulsante Aggiorna.",
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'Scegli la sua cartella skills e Threadnote Office vi scrive il file.',
    ...
    intgExample3: 'Converti budget.xlsx in PDF e aprilo in Threadnote Office',
    intgStep2Note:
      "L'assistente esegue da solo la riga di comando threadnoteoffice; non dovrai mai digitarla.",
    intgCliPartTitle: 'CLI · riga di comando + skill',
    intgCliPartDesc:
      'Per assistenti che possono eseguire comandi da terminale (Claude Code, Codex, Cursor e altri). Installato lo skill, l’assistente richiama da sé la riga di comando threadnoteoffice per creare, convertire, leggere e modificare file; non digiti mai un comando.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP è il protocollo comune con cui gli assistenti IA richiamano strumenti esterni. Usalo quando l’assistente non può eseguire comandi (Claude Desktop, ad esempio) o quando vuoi che controlli direttamente la finestra di Threadnote Office. Scegli uno dei due modi qui sotto.',
    intgMcpStdioTitle: 'Avviato dall’assistente (consigliato)',
    intgMcpStdioDesc:
      'L’assistente avvia threadnoteoffice mcp da sé: niente da attivare qui, e Threadnote Office non deve essere aperto. Stesse funzioni della CLI.',
    ...
    intgCliTitle: 'Avanzato: riga di comando threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · pronto nel terminale ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · non è nel PATH del terminale. Gli assistenti lo trovano comunque tramite ~/.threadnoteoffice/launcher, quindi lo skill funziona. Per digitare threadnoteoffice tu stesso, esegui questo una volta:',
    ...
    starPromptTitleN: 'Hai aperto {n} documenti con Threadnote Office',
    starPromptBody:
      'Threadnote Office è gratuito e open source. Una stella su GitHub è il modo migliore per sostenere il team.',
    starPromptGo: 'Metti una stella',
    starPromptDone: 'Già fatto',
    starPromptLater: 'Più tardi',
    onbStarHint: 'Se ti piace Threadnote Office, lasciaci una stella su GitHub.',
    setEmail: 'Email',
    ...
    newTab: 'Nuova scheda',
    // First-run onboarding
    onbTitle1: 'Benvenuto in Threadnote Office',
  ...
  pl: {
    ...
    intgHeroTitle: 'Używaj Threadnote Office ze swojego asystenta AI',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Gdy aktualizacja Threadnote Office przyniesie nowszy skill, obok tego asystenta pojawi się przycisk Aktualizuj.',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'Wybierz jego folder skills, a Threadnote Office zapisze tam plik.',
    ...
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP to wspólny protokół, którym asystenci AI wywołują zewnętrzne narzędzia. Użyj go, gdy asystent nie potrafi uruchamiać poleceń (np. Claude Desktop) lub gdy chcesz, aby bezpośrednio sterował oknem Threadnote Office. Wybierz jeden z dwóch sposobów poniżej.',
    intgMcpStdioTitle: 'Uruchamiany przez asystenta (zalecane)',
    intgMcpStdioDesc:
      'Asystent sam uruchamia threadnoteoffice mcp: nic tu nie trzeba włączać, a Threadnote Office nie musi być otwarty. Te same funkcje co CLI.',
    intgMcpHttpTitle: 'Lokalny serwer HTTP',
    intgMcpHttpDesc:
      'Działa wewnątrz Threadnote Office; asystent łączy się przez URL, a Ty widzisz, jak dokument powstaje krok po kroku w edytorze Word. Threadnote Office musi pozostać otwarty; na razie tylko dokumenty Word.',
    intgMcpHttpAdvanced: 'Zaawansowane: inne adresy i dziennik',
    intgMcpTryNote: 'Asystent wywołuje Threadnote Office przez MCP; nigdy nie wpisujesz polecenia.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'Zaawansowane: wiersz poleceń threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · gotowy w terminalu ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · nie ma go w PATH terminala. Asystenty i tak znajdą go przez ~/.threadnoteoffice/launcher, więc skill działa. Aby samodzielnie wpisywać threadnoteoffice, uruchom to raz:',
    ...
    starPromptTitleN: 'Otwarto już {n} dokumentów w Threadnote Office',
    starPromptBody:
      'Threadnote Office jest darmowy i open source. Gwiazdka na GitHubie to najlepsze wsparcie dla zespołu.',
    starPromptGo: 'Daj gwiazdkę',
    starPromptDone: 'Już zrobione',
    starPromptLater: 'Później',
    onbStarHint: 'Jeśli podoba Ci się Threadnote Office, zostaw nam gwiazdkę na GitHubie.',
    setEmail: 'E-mail',
    ...
    newTab: 'Nowa karta',
    // First-run onboarding
    onbTitle1: 'Witamy w Threadnote Office',
  ...
  cs: {
    ...
    intgHeroTitle: 'Používejte Threadnote Office ze svého AI asistenta',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Když aktualizace Threadnote Office přinese novější skill, u daného asistenta se objeví tlačítko Aktualizovat.',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'Vyberte jeho složku skills a Threadnote Office tam soubor zapíše.',
    ...
    intgExample3: 'Převeď budget.xlsx do PDF a otevři ho v Threadnote Office',
    intgStep2Note: 'Asistent spouští příkazový řádek threadnoteoffice sám; nikdy ho nemusíte psát.',
    intgCliPartTitle: 'CLI · příkazový řádek + skill',
    intgCliPartDesc:
      'Pro asistenty, kteří umí spouštět příkazy terminálu (Claude Code, Codex, Cursor a další). Po instalaci skillu asistent sám volá příkazový řádek threadnoteoffice k vytváření, převodu, čtení a úpravám souborů; žádný příkaz nepíšete.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP je společný protokol, kterým AI asistenti volají externí nástroje. Použijte ho, když asistent neumí spouštět příkazy (např. Claude Desktop) nebo když chcete, aby přímo ovládal okno Threadnote Office. Vyberte jeden ze dvou způsobů níže.',
    intgMcpStdioTitle: 'Spouští asistent (doporučeno)',
    intgMcpStdioDesc:
      'Asistent spustí threadnoteoffice mcp sám: tady není co zapínat a Threadnote Office nemusí být otevřený. Stejné funkce jako CLI.',
    intgMcpHttpTitle: 'Místní HTTP server',
    intgMcpHttpDesc:
      'Běží uvnitř Threadnote Office; asistent se připojí přes URL a v editoru Word sledujete, jak dokument krok za krokem vzniká. Threadnote Office musí zůstat otevřený; zatím jen dokumenty Word.',
    intgMcpHttpAdvanced: 'Pokročilé: další adresy a protokol',
    intgMcpTryNote: 'Asistent volá Threadnote Office přes MCP; žádný příkaz nepíšete.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'Pokročilé: příkazový řádek threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · připraven v terminálu ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · není v PATH terminálu. Asistenti ho i tak najdou přes ~/.threadnoteoffice/launcher, skill tedy funguje. Chcete-li threadnoteoffice psát sami, spusťte jednou:',
    ...
    starPromptTitleN: 'V Threadnote Office jste otevřeli už {n} dokumentů',
    starPromptBody:
      'Threadnote Office je zdarma a open source. Hvězdička na GitHubu je nejlepší způsob, jak tým podpořit.',
    starPromptGo: 'Dát hvězdičku na GitHubu',
    starPromptDone: 'Hvězdička už udělena',
    starPromptLater: 'Možná později',
    onbStarHint: 'Pokud se vám Threadnote Office líbí, dejte nám hvězdičku na GitHubu.',
    setEmail: 'E-mail',
    ...
    newTab: 'Nová karta',
    // First-run onboarding
    onbTitle1: 'Vítejte v Threadnote Office',
  ...
  nl: {
    ...
    intgHeroTitle: 'Threadnote Office gebruiken vanuit uw AI-assistent',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Wanneer een Threadnote Office-update een nieuwere skill meebrengt, verschijnt naast die assistent een knop Bijwerken.',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'Kies zijn skills-map en Threadnote Office schrijft het bestand daar.',
    ...
    intgExample3: 'Zet budget.xlsx om naar PDF en open het in Threadnote Office',
    intgStep2Note:
      'De assistent voert de threadnoteoffice-opdrachtregel zelf uit; u hoeft die nooit te typen.',
    intgCliPartTitle: 'CLI · opdrachtregel + skill',
    intgCliPartDesc:
      'Voor assistenten die terminalopdrachten kunnen uitvoeren (Claude Code, Codex, Cursor en andere). Na installatie van de skill roept de assistent zelf de threadnoteoffice-opdrachtregel aan om bestanden te maken, converteren, lezen en bewerken; je typt nooit een opdracht.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP is het gemeenschappelijke protocol waarmee AI-assistenten externe tools aanroepen. Gebruik het als je assistent geen opdrachten kan uitvoeren (bijvoorbeeld Claude Desktop) of als je wilt dat hij het Threadnote Office-venster rechtstreeks bestuurt. Kies een van de twee manieren hieronder.',
    intgMcpStdioTitle: 'Gestart door de assistent (aanbevolen)',
    intgMcpStdioDesc:
      'De assistent start threadnoteoffice mcp zelf: hier hoeft niets aan, en Threadnote Office hoeft niet open te staan. Dezelfde functies als de CLI.',
    intgMcpHttpTitle: 'Lokale HTTP-server',
    intgMcpHttpDesc:
      'Draait in Threadnote Office; de assistent verbindt via een URL en je ziet het document stap voor stap ontstaan in de Word-editor. Threadnote Office moet open blijven; voorlopig alleen Word-documenten.',
    intgMcpHttpAdvanced: 'Geavanceerd: andere adressen en logboek',
    intgMcpTryNote: 'De assistent roept Threadnote Office aan via MCP; je typt nooit een opdracht.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'Geavanceerd: threadnoteoffice-opdrachtregel',
    intgCliReady: 'threadnoteoffice {v} · klaar in uw terminal ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · staat niet in het PATH van uw terminal. Assistenten vinden het toch via ~/.threadnoteoffice/launcher, dus de skill werkt. Om zelf threadnoteoffice te typen, voert u dit één keer uit:',
    ...
    starPromptTitleN: 'Je hebt al {n} documenten geopend met Threadnote Office',
    starPromptBody:
      'Threadnote Office is gratis en open source. Een ster op GitHub is de beste steun voor het team.',
    starPromptGo: 'Geef een ster',
    starPromptDone: 'Al gedaan',
    starPromptLater: 'Later',
    onbStarHint: 'Als Threadnote Office je bevalt, geef ons dan een ster op GitHub.',
    setEmail: 'E-mail',
    ...
    // First-run onboarding
    onbTitle1: 'Welkom bij Threadnote Office',
  ...
  ms: {
    ...
    setSecAbout: 'Perihal',
    setSecIntegrations: 'Integrasi',
    intgHeroTitle: 'Gunakan Threadnote Office daripada pembantu AI anda',
    intgHeroDesc:
    ...
    intgStep1Update:
      'Apabila kemas kini Threadnote Office membawa skill yang lebih baharu, butang Kemas kini muncul di sebelah pembantu itu.',
    intgSkillVersion: 'skill {v}',
    ...
    intgExample3: 'Tukar budget.xlsx kepada PDF dan buka dalam Threadnote Office',
    intgStep2Note:
      'Pembantu menjalankan baris arahan threadnoteoffice sendiri; anda tidak perlu menaipnya.',
    intgCliPartTitle: 'CLI · baris arahan + skill',
    intgCliPartDesc:
      'Untuk pembantu yang boleh menjalankan arahan terminal (Claude Code, Codex, Cursor dan lain-lain). Setelah skill dipasang, pembantu memanggil baris arahan threadnoteoffice sendiri untuk mencipta, menukar, membaca dan mengedit fail; anda tidak perlu menaip sebarang arahan.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP ialah protokol umum yang digunakan pembantu AI untuk memanggil alat luaran. Gunakannya apabila pembantu anda tidak boleh menjalankan arahan (contohnya Claude Desktop) atau apabila anda mahu ia mengawal tetingkap Threadnote Office secara langsung. Pilih salah satu daripada dua cara di bawah.',
    intgMcpStdioTitle: 'Dimulakan oleh pembantu (disyorkan)',
    intgMcpStdioDesc:
      'Pembantu melancarkan threadnoteoffice mcp sendiri: tiada apa yang perlu dihidupkan di sini, dan Threadnote Office tidak perlu dibuka. Ciri sama seperti CLI.',
    intgMcpHttpTitle: 'Pelayan HTTP tempatan',
    intgMcpHttpDesc:
      'Berjalan di dalam Threadnote Office; pembantu menyambung melalui URL dan anda boleh melihat dokumen dibina langkah demi langkah dalam editor Word. Threadnote Office mesti kekal dibuka; buat masa ini dokumen Word sahaja.',
    intgMcpHttpAdvanced: 'Lanjutan: alamat lain dan log',
    intgMcpTryNote:
      'Pembantu memanggil Threadnote Office melalui MCP; anda tidak perlu menaip sebarang arahan.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'Lanjutan: baris arahan threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · sedia dalam terminal anda ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · tiada dalam PATH terminal anda. Pembantu masih menemuinya melalui ~/.threadnoteoffice/launcher, jadi skill berfungsi. Untuk menaip threadnoteoffice sendiri, jalankan ini sekali:',
    ...
    starPromptTitleN: 'Anda telah membuka {n} dokumen dengan Threadnote Office',
    starPromptBody:
      'Threadnote Office percuma dan sumber terbuka. Bintang di GitHub ialah sokongan terbaik untuk pasukan kami.',
    starPromptGo: 'Beri Bintang',
    starPromptDone: 'Sudah',
    starPromptLater: 'Kemudian',
    onbStarHint: 'Jika anda suka Threadnote Office, beri kami bintang di GitHub.',
    setEmail: 'E-mel',
    ...
    newTab: 'Tab baharu',
    // First-run onboarding
    onbTitle1: 'Selamat datang ke Threadnote Office',
  ...
  he: {
    ...
    intgHeroTitle: 'שימוש ב-Threadnote Office מתוך עוזר ה-AI שלכם',
    intgHeroDesc:
    ...
    intgOtherFolderDesc: 'בחרו את תיקיית ה-skills שלו ו-Threadnote Office יכתוב שם את הקובץ.',
    ...
    intgExample3: 'המר את budget.xlsx ל-PDF ופתח אותו ב-Threadnote Office',
    intgStep2Note: 'העוזר מריץ בעצמו את שורת הפקודה threadnoteoffice; לעולם לא תצטרכו להקליד אותה.',
    intgCliPartTitle: 'CLI · שורת פקודה + skill',
    intgCliPartDesc:
      'לעוזרים שיכולים להריץ פקודות טרמינל (Claude Code‏, Codex‏, Cursor ואחרים). לאחר התקנת ה‑skill העוזר קורא בעצמו לשורת הפקודה threadnoteoffice כדי ליצור, להמיר, לקרוא ולערוך קבצים; אתם לא מקלידים שום פקודה.',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP הוא הפרוטוקול המשותף שבו עוזרי AI קוראים לכלים חיצוניים. השתמשו בו כשהעוזר אינו יכול להריץ פקודות (למשל Claude Desktop) או כשאתם רוצים שהוא ישלוט ישירות בחלון Threadnote Office. בחרו אחת משתי הדרכים למטה.',
    intgMcpStdioTitle: 'מופעל על ידי העוזר (מומלץ)',
    intgMcpStdioDesc:
      'העוזר מפעיל את threadnoteoffice mcp בעצמו: אין מה להפעיל כאן, ו‑Threadnote Office לא חייב להיות פתוח. אותן יכולות כמו ב‑CLI.',
    intgMcpHttpTitle: 'שרת HTTP מקומי',
    intgMcpHttpDesc:
      'פועל בתוך Threadnote Office; העוזר מתחבר דרך כתובת URL ואפשר לראות את המסמך נבנה צעד אחר צעד בעורך Word. Threadnote Office חייב להישאר פתוח; בשלב זה מסמכי Word בלבד.',
    intgMcpHttpAdvanced: 'מתקדם: כתובות נוספות ויומן',
    intgMcpTryNote: 'העוזר קורא ל‑Threadnote Office דרך MCP; אתם לא מקלידים שום פקודה.',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'מתקדם: שורת הפקודה threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · מוכן בטרמינל ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · לא נמצא ב-PATH של הטרמינל. העוזרים עדיין מוצאים אותו דרך ~/.threadnoteoffice/launcher, ולכן ה-skill עובד. כדי להקליד threadnoteoffice בעצמכם, הריצו זאת פעם אחת:',
    ...
    intgCopied: 'הועתק',
    intgCliEphemeral: 'גררו קודם את Threadnote Office לתיקיית היישומים, ואז השתמשו בשורת הפקודה.',
    intgCopyPath: 'העתקת נתיב',
    intgCliNeedsUpdate: 'skill זה דורש שורת פקודה {v} ומעלה; עדכנו את Threadnote Office.',
    intgLoading: 'בודק…',
    ...
    onbStarHint: 'אם Threadnote Office מוצא חן בעיניכם, תנו לנו כוכב ב-GitHub.',
    setEmail: 'אימייל',
    ...
    // First-run onboarding
    onbTitle1: 'ברוכים הבאים ל-Threadnote Office',
  ...
  hi: {
    ...
    intgHeroTitle: 'अपने AI असिस्टेंट से Threadnote Office इस्तेमाल करें',
    intgHeroDesc:
    ...
    intgStep1Update:
      'जब Threadnote Office के अपडेट में नया skill आता है, तो उस असिस्टेंट के पास अपडेट बटन दिखता है।',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: 'उसका skills फ़ोल्डर चुनें, Threadnote Office वहाँ फ़ाइल लिख देगा।',
    ...
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP वह साझा प्रोटोकॉल है जिससे AI सहायक बाहरी टूल बुलाते हैं। इसे तब चुनें जब सहायक कमांड न चला सके (जैसे Claude Desktop) या आप चाहें कि वह Threadnote Office की विंडो को सीधे चलाए। नीचे दिए दो तरीकों में से कोई एक चुनें।',
    intgMcpStdioTitle: 'सहायक द्वारा शुरू (अनुशंसित)',
    intgMcpStdioDesc:
      'सहायक खुद threadnoteoffice mcp चलाता है: यहाँ कुछ चालू करने की ज़रूरत नहीं, और Threadnote Office खुला होना भी ज़रूरी नहीं। सुविधाएँ CLI जैसी ही।',
    intgMcpHttpTitle: 'स्थानीय HTTP सर्वर',
    intgMcpHttpDesc:
      'Threadnote Office के अंदर चलता है; सहायक URL से जुड़ता है और आप Word संपादक में दस्तावेज़ को चरण-दर-चरण बनते देख सकते हैं। Threadnote Office खुला रहना चाहिए; अभी केवल Word दस्तावेज़।',
    intgMcpHttpAdvanced: 'उन्नत: अन्य पते और लॉग',
    intgMcpTryNote:
      'सहायक MCP के ज़रिये Threadnote Office को बुलाता है; आपको कोई कमांड टाइप नहीं करना पड़ता।',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: 'उन्नत: threadnoteoffice कमांड लाइन',
    intgCliReady: 'threadnoteoffice {v} · टर्मिनल में तैयार ({path})',
    intgCliNotOnPath:
      'threadnoteoffice {v} · टर्मिनल के PATH में नहीं है। असिस्टेंट इसे ~/.threadnoteoffice/launcher से फिर भी ढूँढ लेते हैं, इसलिए skill काम करता है। खुद threadnoteoffice टाइप करने के लिए यह एक बार चलाएँ:',
    ...
    starPromptTitleN: 'आपने Threadnote Office में {n} दस्तावेज़ खोले हैं',
    starPromptBody:
      'Threadnote Office मुफ़्त और ओपन सोर्स है। GitHub पर एक स्टार टीम का सबसे अच्छा समर्थन है।',
    starPromptGo: 'स्टार दें',
    starPromptDone: 'पहले ही दे दिया',
    starPromptLater: 'बाद में',
    onbStarHint: 'अगर आपको Threadnote Office पसंद है, तो हमें GitHub पर स्टार दें।',
    setEmail: 'ईमेल',
    ...
    newTab: 'नया टैब',
    // First-run onboarding
    onbTitle1: 'Threadnote Office में आपका स्वागत है',
  ...
  'zh-TW': {
    ...
    intgHeroTitle: '在 AI 助理中使用 Threadnote Office',
    intgHeroDesc:
    ...
    intgStep1Update: 'Threadnote Office 升級帶來新版 skill 時，對應助理旁會出現「更新」按鈕。',
    intgSkillVersion: 'skill {v}',
    ...
    intgOtherFolderDesc: '選擇它的 skills 資料夾，Threadnote Office 會把檔案寫進去。',
    ...
    intgExample3: '把 budget.xlsx 轉成 PDF，並在 Threadnote Office 中開啟',
    intgStep2Note: '助理會自行呼叫 threadnoteoffice 命令列，你不需要輸入任何指令。',
    intgCliPartTitle: 'CLI · 命令列 + skill',
    intgCliPartDesc:
      '適合能執行終端機指令的助理（Claude Code、Codex、Cursor 等）。安裝 skill 後，助理會自己呼叫 threadnoteoffice 命令列建立、轉換、讀取和編輯檔案，你不需要輸入任何指令。',
    intgMcpPartTitle: 'MCP',
    intgMcpPartDesc:
      'MCP 是 AI 助理呼叫外部工具的通用協定。助理不能執行指令（如 Claude Desktop），或你想讓它直接操作 Threadnote Office 視窗時用這條路。下面兩種接法任選一種。',
    intgMcpStdioTitle: '由助理啟動（建議）',
    intgMcpStdioDesc:
      '助理自己啟動 threadnoteoffice mcp，不需要在這裡開任何開關，Threadnote Office 也不用開著。功能與 CLI 完全相同。',
    intgMcpHttpTitle: '本機 HTTP 服務',
    intgMcpHttpDesc:
      '在 Threadnote Office 內執行，助理透過網址連線，可以看著它在 Word 編輯器裡逐步產生。需要 Threadnote Office 保持開啟；目前只支援 Word。',
    intgMcpHttpAdvanced: '進階：其他位址與記錄',
    intgMcpTryNote: '助理會透過 MCP 呼叫 Threadnote Office，你不需要輸入任何指令。',
    intgMcpClaudeTitle: 'Claude Code',
    ...
    intgCliTitle: '進階：命令列 threadnoteoffice',
    intgCliReady: 'threadnoteoffice {v} · 已可在終端機直接使用（{path}）',
    intgCliNotOnPath:
      'threadnoteoffice {v} · 尚未加入終端機 PATH。助理仍能透過 ~/.threadnoteoffice/launcher 找到它，skill 不受影響。若想自己在終端機輸入 threadnoteoffice，執行一次：',
    ...
    intgCopied: '已複製',
    intgCliEphemeral: '請先把 Threadnote Office 拖到「應用程式」，再使用命令列。',
    intgCopyPath: '複製路徑',
    intgCliNeedsUpdate: '此 skill 需要命令列 {v} 或更新版本，請更新 Threadnote Office。',
    intgLoading: '正在檢查…',
    intgUpdateDue: 'skill 有新版本可更新',
    setSecAiModel: 'AI 模型',
    setAiProvider: '模型服務商',
    ...
    onbStarHint: '如果你喜歡 Threadnote Office，歡迎到 GitHub 給我們一顆星。',
    setEmail: '電子郵件',
    setNotLoggedIn: '未登入',
    setViewUsage: '查看用量',
    setChange: '變更',
    ...
    newTab: '新分頁',
    // First-run onboarding
    onbTitle1: '歡迎使用 Threadnote Office',
