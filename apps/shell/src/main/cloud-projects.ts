...
import { gskApiKey, gskListPastProjects, hasGskAuth } from '@threadnote/ai-search'
...
export function cloudStoreOwner(): string {
  const key = gskApiKey()
  if (!key) return ''
  return createHash('sha256').update(`threadnoteoffice-cloud-store:${key}`).digest('hex').slice(0, 16)
}
