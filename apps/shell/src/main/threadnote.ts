...
async function chooseProject(
  projects: ThreadnoteProject[],
  fileName: string,
): Promise<ThreadnoteProject | null> {
...
const input=document.querySelector('input'),buttons=[...document.querySelectorAll('button')],empty=document.querySelector('#empty');input.addEventListener('input',()=>{const query=input.value.trim().toLowerCase();let shown=0;for(const button of buttons){const visible=button.textContent.toLowerCase().includes(query);button.style.display=visible?'flex':'none';if(visible)shown++}empty.style.display=shown?'none':'block'});for(const button of buttons)button.addEventListener('click',()=>location.href='threadnote-share://select?id='+button.dataset.id);</script></body></html>`
  return new Promise((resolve, reject) => {
    ...
    chooser
      ...
      .then(() => {
        if (!chooser.isDestroyed()) chooser.show()
      })
      .catch(reject)
  })
}
...
function trackManagedFile(filePath: string, file: ThreadnoteFile): void {
  ...
  const save = async () => {
    ...
    try {
      ...
      const updated = await api<ThreadnoteFile>(
        auth.baseUrl,
        `/api/office-files/${encodeURIComponent(managed.fileId)}/content`,
        {
          method: 'PUT',
      ...
      await dialog.showMessageBox({
        type: 'error',
        title: 'Threadnote sync failed',
        message,
        detail: 'Your local file is still saved.',
        buttons: ['Close'],
      })
    } finally {
...
async function openManagedFile(fileId: string, openPath: (path: string) => boolean): Promise<void> {
  ...
  const details = await api<ThreadnoteFile>(
    auth.baseUrl,
    `/api/office-files/${encodeURIComponent(fileId)}`,
    {},
    auth.token,
  )
  const response = await fetch(
    auth.baseUrl + `/api/office-files/${encodeURIComponent(fileId)}/content`,
    {
...
export async function openThreadnoteUrl(
  ...
  if (url.protocol !== 'threadnote:' || url.hostname !== 'open')
    throw new Error('Invalid Threadnote link.')
...
export function startThreadnoteBridge(openPath: (path: string) => boolean): void {
  ...
  localBridge = createServer((request, response) => {
    ...
    try {
      ...
      allowed =
        origin === configured || ['http://localhost:5173', 'http://127.0.0.1:5173'].includes(origin)
    } catch {
      allowed = false
    }
...
export function registerThreadnoteIpc(
  openPath: (path: string) => boolean,
  ...
  ipcMain.handle(THREADNOTE_CHANNELS.status, async (): Promise<ThreadnoteStatus> => {
    const auth = readAuth()
    return {
      connected: Boolean(decodedToken(auth)),
      baseUrl: auth.baseUrl,
      available: await officeAvailable(auth.baseUrl),
    }
  })
  ipcMain.handle(THREADNOTE_CHANNELS.startPairing, async (_event, rawBaseUrl: unknown) => {
    const baseUrl = normalizeBaseUrl(rawBaseUrl)
    if (!(await officeAvailable(baseUrl)))
      throw new Error('Threadnote Office sharing is not available on this server yet.')
  ...
  ipcMain.handle(
    THREADNOTE_CHANNELS.files,
    async (_event, projectId: unknown): Promise<ThreadnoteFile[]> => {
      const auth = authenticated()
      const query =
  ...
  ipcMain.handle(THREADNOTE_CHANNELS.shareCurrentFile, async (): Promise<ThreadnoteFile | null> => {
    try {
      ...
      const auth = authenticated()
      if (!(await officeAvailable(auth.baseUrl)))
        throw new Error('Threadnote Office sharing is not available on this server yet.')
      ...
      if (writable.length === 0)
        throw new Error('You do not have a Threadnote project that accepts uploads.')
      ...
      await dialog.showMessageBox({
        type: 'info',
        title: 'Shared through Threadnote',
        message: `${basename(filePath)} was shared`,
      ...
      await dialog.showMessageBox({
        type: 'error',
        title: 'Threadnote sharing failed',
        message,
        buttons: ['Close'],
      })
      throw cause
