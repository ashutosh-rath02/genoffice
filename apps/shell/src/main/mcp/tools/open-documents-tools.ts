...
function noMatch(target: string, documents: readonly OpenDocumentTab[]): Error {
  if (documents.length === 0) {
    return new Error(`no document is open in ThreadnoteOffice, so "${target}" cannot be resolved`)
  }
  ...
  return [
    {
      name: 'open_documents',
      description:
        'Work with the documents the user currently has open in ThreadnoteOffice tabs — not just the ' +
      ...
      handler: async (args) => {
        if (!control) {
          throw new Error('ThreadnoteOffice is not running, so its open documents are unreachable')
        }
