...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'threadnoteoffice-sheets-temp-test-'))
  roots.push(root)
  return root
}
...
describe('session temporary resources', () => {
  it('closes the sidecar before removing the snapshot and converted import directory', async () => {
    ...
    const snapshotDir = join(root, 'threadnoteoffice-sheets-sessions')
    const importDir = join(root, 'threadnoteoffice-imports', randomUUID())
...
describe('pasted-image TTL cleanup', () => {
  it('removes only expired app-owned files from the direct pasted temp directory', async () => {
    const root = await tempRoot()
    const pastedDir = join(root, 'threadnoteoffice-pasted')
