...
export async function cleanupSessionResources(
  ...
  const snapshotRoot = join(options.tempRoot, 'threadnoteoffice-sheets-sessions')
...
export async function cleanupImportTempDirectory(
  ...
  const importRoot = join(tempRoot, 'threadnoteoffice-imports')
...
export async function cleanupExpiredPastedFiles(
  ...
  const dir = join(tempRoot, 'threadnoteoffice-pasted')
