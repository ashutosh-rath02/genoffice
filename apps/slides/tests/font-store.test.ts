...
beforeEach(() => {
  availability.clear()
  vi.stubEnv('THREADNOTE_OFFICE_FONT_CDN_URL', fontCdnBaseUrl)
...
describe('font catalog', () => {
  ...
  it('hides the downloadable catalog when no CDN URL is configured', () => {
    vi.stubEnv('THREADNOTE_OFFICE_FONT_CDN_URL', '')
  ...
  it('extracts and validates the packaged CDN URL', () => {
    expect(
      extractFontCdnBaseUrl({
        threadnoteofficeFontCdn: { baseUrl: ' https://fonts.example.test/v1/ ' },
      }),
    ).toBe(fontCdnBaseUrl)
    expect(
    ...
    expect(
      extractFontCdnBaseUrl({
        threadnoteofficeFontCdn: { baseUrl: 'https://user@fonts.example.test/v1' },
      }),
    ).toBeNull()
    expect(
      extractFontCdnBaseUrl({
        threadnoteofficeFontCdn: { baseUrl: 'https://fonts.example.test/v1?token=secret' },
      }),
    ).toBeNull()
  })
})
