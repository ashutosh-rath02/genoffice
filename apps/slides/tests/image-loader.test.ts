...
vi.mock('@threadnote/docx-engine/metafile', () => ({
...
describe('metafile rasterization waits for private fonts', () => {
  const flag = window as { __threadnoteofficeDocFontsSynced?: boolean }
  beforeEach(() => {
    FakeImage.instances = []
    vi.stubGlobal('Image', FakeImage)
    vi.useFakeTimers()
  ...
  afterEach(() => {
    vi.unstubAllGlobals()
    vi.useRealTimers()
    delete flag.__threadnoteofficeDocFontsSynced
  })

  it('holds the EMF until the doc-fonts sync flag flips, then rasterizes', async () => {
    flag.__threadnoteofficeDocFontsSynced = false
    ...
    flag.__threadnoteofficeDocFontsSynced = true
