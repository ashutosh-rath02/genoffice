...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'threadnoteoffice-slides-temp-test-'))
  roots.push(root)
  return root
}
...
describe('generated-page temp sweep', () => {
  it('removes only expired .pptx files matching the UUID pattern', async () => {
    ...
    const cloudDir = join(root, 'threadnoteoffice-cloud-pages')
    const localDir = join(root, 'threadnoteoffice-local-pages')
  ...
  it('skips directories and non-.pptx files', async () => {
    const root = await tempRoot()
    await mkdir(join(root, 'threadnoteoffice-cloud-pages'), { recursive: true })

    const dirLike = join(root, 'threadnoteoffice-cloud-pages', 'not-a-file')
