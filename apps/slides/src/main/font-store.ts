 ...
 *
 * A curated catalog of OFL-licensed families is mirrored on the ThreadnoteOffice CDN
...
import type { OpenedPptx } from '@threadnote/pptx-engine'
...
export function extractFontCdnBaseUrl(pkg: unknown): string | null {
  ...
  const raw = (pkg as Record<string, unknown>).threadnoteofficeFontCdn
...
export function fontCdnBaseUrl(): string | null {
  if (!app.isPackaged) return normalizeFontCdnBaseUrl(process.env.THREADNOTE_OFFICE_FONT_CDN_URL)
  try {
