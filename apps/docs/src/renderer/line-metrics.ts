...
import type { DocGrid } from '@threadnote/docx-engine'
...
const KO_FONT_RE =
  /malgun|맑은|batang|바탕|myeongjo|myungjo|명조|gungsuh|궁서|gulim|굴림|dotum|돋움|nanum|나눔|threadnoteoffice (sans|serif) kr|(noto|source han) (sans|serif)[^,]*\bk(r|orean)?\b/i
...
export const BUNDLED_FONTS = new Set([
  ...
  'ThreadnoteOffice Poppins',
  'ThreadnoteOffice Tamil',
  'ThreadnoteOffice Fullwidth TC',
  'ThreadnoteOffice Songti SC',
  'Carlito GO',
  'Aptos GO',
  'Aptos Display GO',
  'Century Gothic GO',
...
export function cssFontFamily(font: string, followAltName = true): string {
  ...
  // 'ThreadnoteOffice Box Drawing' (fonts.css): U+2500 rules would otherwise fall to
  // the fullwidth CJK subset and overflow the column
  const BOX = 'ThreadnoteOffice Box Drawing'
  ...
  // 'ThreadnoteOffice PUA Blank' keeps AI-residue PUA tokens invisible like Word
  ...
  if (f.includes('aptos')) {
    ...
    const alias = /narrow|mono|serif/.test(f)
      ...
      : f.includes('display')
        ? 'Aptos Display GO'
        : 'Aptos GO'
    return `${chain(font, alias, 'ThreadnoteOffice PUA Blank', CJK_SANS)},sans-serif`
  }
  ...
  if (SEGOE_UI_TEXT_RE.test(f))
    return `${chain(font, 'Segoe UI GO', 'Noto Sans CJK GO', 'ThreadnoteOffice PUA Blank')},sans-serif`
  ...
  // 'ThreadnoteOffice Songti SC' (fonts.css local() alias of Songti SC): macOS Chromium
  ...
  if (f.includes('simsun') || f.includes('宋体') || f.includes('nsimsun')) {
    return `${chain(font, 'ThreadnoteOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF)},serif`
  }
  ...
  // 'ThreadnoteOffice *' entries are CJK-only local() aliases (fonts.css): the underlying
  // system faces draw Cyrillic/Greek fullwidth, so those scripts must pass through
  const JA_SANS = ['Yu Gothic', 'ThreadnoteOffice Hiragino Sans', 'Meiryo', 'Noto Sans JP']
  const JA_SERIF = [
    'Yu Mincho',
    'ThreadnoteOffice Hiragino Mincho',
  ...
  if (
    ...
    if (/nanum ?gothic|나눔 ?고딕/i.test(nfkc)) {
      return `${chain(font, 'ThreadnoteOffice Gothic KR', ...KO_SANS)},sans-serif`
    }
    ...
    if (/(?:batang|gulim|dotum|gungsuh) ?che\b|(?:바탕|굴림|돋움|궁서)체/i.test(nfkc)) {
      ...
      return `${chain(font, 'ThreadnoteOffice Che Latin KR', ...(cheGungsuh ? ['GungSeo'] : []), ...(cheSerif ? KO_SERIF : KO_SANS))},${cheSerif ? 'serif' : 'sans-serif'}`
    }
  ...
  if (/nyala|ebrima|abyssinica|ethiopic/i.test(nfkc)) {
    return `${chain(font, 'ThreadnoteOffice Ethiopic')},sans-serif`
  }
  ...
  if (/tamil|latha|vijaya|inaimathi/i.test(nfkc)) {
    return `${chain(font, 'ThreadnoteOffice Tamil', 'InaiMathi', 'Tamil MN', 'Tamil Sangam MN')},sans-serif`
  }
  ...
  const pua = latinNamed ? ['ThreadnoteOffice PUA Blank'] : []
