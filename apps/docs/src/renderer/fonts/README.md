...
to the same faces) and are referenced here as `@threadnote/ui/fonts/Carlito-*.ttf`.
...
The serif subset also backs the `ThreadnoteOffice Fullwidth TC` face (`fonts.css`), a
...
`ThreadnoteOfficeCheLatinKR.woff2` is an ASCII-only derivative of ThreadnoteOffice Sans KR.
...
`ThreadnoteOfficeGothicKR-Regular-subset.woff2`. Renamed per OFL (the upstream
...
checked in as `ThreadnoteOfficePoppins-{Regular,Bold}-subset.woff2`. Renamed (no
