    ...
    for (const sheet of Array.from(document.styleSheets)) {
      ...
      for (const rule of Array.from(cssRules)) {
        ...
        if (
          ...
          ff.style.getPropertyValue('font-family').includes('ThreadnoteOffice PUA Blank')
        ) {
  ...
  if (blankSrcCache == null) {
    ...
    try {
      for (const el of Array.from(document.querySelectorAll('style'))) {
        const m = /@font-face\s*{[^}]*ThreadnoteOffice PUA Blank[^}]*?src:\s*([^;}]+)/.exec(
          el.textContent ?? '',
        )
  ...
  }
  {
    ...
    if (typedGrid) {
      ...
      if (metrics && blankSrc) {
        gridStrut = true
        rules.push(
          `@font-face { font-family:'ThreadnoteOffice Grid Strut'; src:${blankSrc}; ` +
