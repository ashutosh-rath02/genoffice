...
const sans = readWoff2(join(FONTS, 'ThreadnoteOfficeSansKR-Regular-subset.woff2'))
const serif = readWoff2(join(FONTS, 'ThreadnoteOfficeSerifKR-Regular-subset.woff2'))
...
describe('ThreadnoteOffice Sans KR (Malgun-normalized)', () => {
...
describe('ThreadnoteOffice Serif KR (Batang-normalized)', () => {
...
describe('ThreadnoteOffice Che Latin KR (fixed-pitch half-width)', () => {
  const che = readWoff2(join(FONTS, 'ThreadnoteOfficeCheLatinKR.woff2'))
...
describe('ThreadnoteOffice Gothic KR (real source metrics, unmodified)', () => {
  const gothicKr = readWoff2(join(FONTS, 'ThreadnoteOfficeGothicKR-Regular-subset.woff2'))
  ...
  it('keeps every primary font identifier clear of Reserved Font Names', () => {
    ...
    expect(names).toContainEqual({ nameId: 1, value: 'ThreadnoteOffice Gothic KR' })
    expect(names).toContainEqual({ nameId: 6, value: 'ThreadnoteOfficeGothicKR-Regular' })
  })
})
