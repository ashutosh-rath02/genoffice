...
const localAlias = {
  '@threadnote/docx-engine/lazy-media': resolve(
    __dirname,
    '../../packages/docx-engine/src/lazy-media.ts',
  ),
  '@threadnote/docx-engine/zip-splice': resolve(
    __dirname,
    '../../packages/docx-engine/src/zip-splice.ts',
  ),
  '@threadnote/docx-engine': resolve(__dirname, '../../packages/docx-engine/src/index.ts'),
}
...
export default defineConfig({
  ...
  // @threadnote/* deps ship as raw TS source with extensionless imports, so they
  ...
  renderer: {
    ...
    resolve: { alias: localAlias },
    server: {
      // Overridable so multiple threadnoteoffice dev instances can coexist (default 5173).
