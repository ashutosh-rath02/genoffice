...
// the preload exposes window.__threadnoteofficeDebug only under this env var; the
...
async function selectionRect(sheets: Page): Promise<{
  ...
  return sheets.evaluate(() => {
    const debug = (window as unknown as Record<string, unknown>).__threadnoteofficeDebug as {
      univerAPI: {
        getActiveWorkbook(): {
          getActiveRange(): {
            getRange(): {
...
test.describe('sheets: arrow collapses a multi-cell selection to the active cell', () => {
  test('after selecting two cells and cutting, ArrowRight lands next to the active cell', async () => {
    const scratch = await mkdtemp(join(tmpdir(), 'threadnoteoffice-arrow-collapse-'))
