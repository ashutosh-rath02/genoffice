...
async function buildRubikFixture(): Promise<string> {
  const out = join(
    await mkdtemp(join(tmpdir(), 'threadnoteoffice-font-manager-')),
 ...
 * prompts. Providing THREADNOTE_OFFICE_FONT_CDN_URL enables the catalog/banner; the
...
test('font download UI follows CDN configuration', async () => {
  ...
  try {
    ...
    const configured = Boolean(process.env.THREADNOTE_OFFICE_FONT_CDN_URL?.trim())
    if (!configured) {
